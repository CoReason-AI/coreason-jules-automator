...
def test_markdown_reporter_generates_report(tmp_path: Path) -> None:
    ...
    events = [
        ...
        AutomationEvent(type=EventType.CYCLE_START, message="End", timestamp=1003.0), # using cycle start as placeholder for end
    ]
...
def test_markdown_reporter_renders_full_template(tmp_path: Path) -> None:
    ...
    events = [
        ...
        AutomationEvent(type=EventType.CHECK_RESULT, message="CI Build", payload={"status": "fail", "url": "http://ci"}),
