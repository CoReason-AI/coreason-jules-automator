...
async def test_ci_polling_step_fetch_error() -> None:
    ...
    with (
        patch("coreason_jules_automator.strategies.steps.logger") as mock_logger,
...
async def test_ci_polling_step_checks_not_completed() -> None:
    ...
    mock_github.get_pr_checks = AsyncMock(
        return_value=[PullRequestStatus(name="c1", status="in_progress", conclusion=None, url="u1")]
    )
...
async def test_log_analysis_step_no_failed_check_found() -> None:
    ...
    context = OrchestrationContext(
        ...
        pipeline_data={
            "ci_passed": False,
            "ci_checks": [PullRequestStatus(name="c1", status="completed", conclusion="success", url="u1")],
        },
    )
